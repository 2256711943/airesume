...
test.describe('login flow', () => {
  ...
  test('logs in and redirects to resume', async ({ page }) => {
    ...
    await expect(page.locator('header.workspace-topbar h1')).toHaveText('Agent');
    await expect(page.locator('main .editor-panel h2')).toBeVisible();
    await expect(page.locator('body')).toContainText('Demo User');
    await expect(page.locator('body')).toContainText('user@example.com');

    const storedToken = await page.evaluate(() => localStorage.getItem('aitext_access_token'));
