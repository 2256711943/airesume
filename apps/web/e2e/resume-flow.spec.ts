...
const setupConversationMocks = async (
  page: Page,
...
const fillResumeForm = async (page: Page) => {
  ...
  await page
    .locator(".form-grid .field textarea")
    .first()
    .fill("Five years of backend experience in SaaS.");
...
test.describe("resume page flow", () => {
  ...
  test("clears an invalid persisted conversation id but keeps the local draft", async ({
    page,
  }) => {
    ...
    await page.route(
      "**/conversations/conv-stale-1/resume-session**",
      async (route) => {
        restoreAttempts += 1;
    ...
    await expect(page.locator(".form-grid .field input").nth(0)).toHaveValue(
      "Alex Chen",
    );
    await expect(page.locator(".form-grid .field input").nth(1)).toHaveValue(
      "Backend Engineer",
    );
    await expect(page.locator(".variant-summary-text")).toContainText(
      "Recovered draft summary.",
    );
  ...
  test("generates three resume variants successfully", async ({ page }) => {
    ...
    await page.route("**/resume/generate/stream**", async (route) => {
      await fulfillSse(route, [
        ...
        {
          event: "progress",
          data: { taskId: "task-1", progress: 35, stage: "planning" },
        },
        {
          event: "chunk",
          data: {
            taskId: "task-1",
            text: "Drafting tailored resume bullets...",
          },
        },
        {
          event: "progress",
          data: { taskId: "task-1", progress: 100, stage: "post_processing" },
        },
        {
          event: "done",
          data: {
                ...
                experience: [
                  {
                    ...
                    role: "Backend Engineer",
                    highlights: [
                      "Built APIs for a multi-tenant platform",
                ...
                projects: [
                  {
                    name: "Resume Assistant",
                    highlights: [
                      "Raised conversion from trial to consultation",
                    ],
                  },
                ],
                ...
                experience: [
                  {
                    ...
                    role: "Backend Engineer",
                    highlights: [
                      "Balanced delivery speed with reliability goals",
                    ],
                  },
                ],
                projects: [
                  {
                    name: "Resume Assistant",
                    highlights: [
                      "Connected product goals with engineering output",
                    ],
                  },
                ],
  ...
  test("sends a chat message and creates a conversation", async ({ page }) => {
    ...
    const assistantMessage =
      "Focus on measurable impact, ordering, and clearer project outcomes.";
    ...
    await expect(
      page.locator(".chat-message.user .plain-message").last(),
    ).toContainText(userMessage);
    await expect(
      page.locator(".chat-message.assistant .markdown-body").last(),
    ).toContainText(assistantMessage);
