...
import type { SpanDerivedAnomaly } from './useSpanStore';

function createEvent(eventId: string, seq: number, type: string, spanId: string | null): SpanEvent {
  return {
    eventId,
    seq,
    runId: 'run-1',
...
function createAnomaly(overrides: Partial<SpanDerivedAnomaly> & { anomalyId: string }): SpanDerivedAnomaly {
  return {
    kind: 'tool_failure',
    severity: 'warning',
    title: '异常',
...
function createIssue(overrides: Partial<ObservabilityDiagnosticIssue> & { issueId: string }): ObservabilityDiagnosticIssue {
  return {
    runId: 'run-1',
    spanId: null,
    category: 'other',
...
describe('toDiagnosticItems', () => {
  ...
  it('builds evidence from anomaly event id', () => {
    const [item] = toDiagnosticItems([
      createAnomaly({ anomalyId: 'a1', kind: 'tool_timeout', eventId: 'evt-3', reason: '超时' }),
    ]);
...
describe('useObservabilityDiagnostics', () => {
  ...
  it('merges remote issues and local anomalies, remote wins on duplicate span', () => {
    const remoteIssues: ObservabilityDiagnosticIssue[] = [
      createIssue({
        ...
        spanId: 's-tool',
        severity: 'critical',
        evidence: [{ eventId: 'evt-4', type: 'tool.call.finished', value: 'x' }],
      }),
      createIssue({
        issueId: 'issue-2',
    ...
    // 被去重的本地异常不出现
    expect(diagnostics.items.value.some((item) => item.id === 'anomaly-1')).toBe(false);
  });
  ...
  it('groups by severity and category with counts', () => {
    ...
    expect(diagnostics.counts.value).toEqual({ total: 2, critical: 1, warning: 1, info: 0 });

    expect(diagnostics.groupedBySeverity.value.map((group) => group.key)).toEqual([
      'critical',
      'warning',
    ]);
    expect(diagnostics.groupedBySeverity.value[0].items.map((item) => item.id)).toEqual([
      'issue-1',
    ]);
  ...
  it('returns empty aggregates when no sources', () => {
    ...
    expect(diagnostics.counts.value).toEqual({ total: 0, critical: 0, warning: 0, info: 0 });
  });
});
